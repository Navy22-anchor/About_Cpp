...
a = a+1
```
* 코드가 추구하는 이상이란 복잡함이 아니라 단순함이다.
* switch (a) 의 a는 반드시 정수, char, 열거형 타입 이여야함
* case 라벨은 반드시 상수 표현식이여야함. 변수는 불가
* 두 case 레이블에 같은 값을 지정 불가.
* switch 구문은 상수 집합과 값을 비교하는데 최적이다.
* for 구문의 몸체안에서 루프 변수의 값을 변경하지 말자.
